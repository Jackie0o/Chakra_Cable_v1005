    ...
    (() => {
        ...
        class DynamicAdapt {
            ...
            init() {
                ...
                this.nodes = [ ...document.querySelectorAll("[data-da], [data-da-daddy]") ];
                this.nodes.forEach((node => {
                    if (node.hasAttribute("data-da")) this.createObject(node); else if (node.hasAttribute("data-da-daddy")) this.createObjectForDaddy(node);
                }));
            ...
            createObjectForDaddy(node) {
                const data = node.dataset.daDaddy.trim();
                const dataArray = data.split(",");
                const object = {};
                object.element = node;
            ...
            mediaHandler(matchMedia, objects) {
                if (matchMedia.matches) objects.forEach((object => {
                    if (object.children) object.children.forEach((child => {
                        this.moveTo(object.place, child, object.destination);
                    })); else this.moveTo(object.place, object.element, object.destination);
                })); else objects.forEach((({parent, element, index, children}) => {
                    if (element.classList.contains(this.daClassname)) if (children) children.forEach((child => {
                        this.moveBack(parent, child, this.indexInParent(element, child));
                    })); else this.moveBack(parent, element, index);
                }));
            }
